...
def take_turn(board, turn_number, player):
    ...
    for coordinate, specimens in board.specimens.items():
        for specimen in specimens:
            ...
            new_vision = [[board.get_color(teleported+offset)
                      for offset in line] for line in VISION]




            if teleported.x > coordinate.x:
                rew += 10
            elif teleported.x == coordinate.x:
                rew += -5
            else:
                rew += -10
            ...
            if board.get_square(teleported).killer:
                rew = -5
                teleported = random.choice(board.starting_squares)
            ...
            if teleported.at_finish():
                rew = 10
                done = 1
            player.get_data(vision,new_vision,rew,done)
